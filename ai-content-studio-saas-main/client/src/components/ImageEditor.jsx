...
export default function ImageEditor({ imageId, imageUrl, onClose, onSaved }) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `edited-${imageId.slice(0, 8)}.png`;
      a.click();
        ...
        <div className="relative max-w-full max-h-full">
          {holdOriginal ? (
            <div className="relative">
              ...
              />
              <div className="absolute top-3 left-3 px-2.5 py-1 rounded-md text-[11px] font-bold bg-black/60 text-amber-400 backdrop-blur-sm">
                ORIGINAL
              </div>
            </div>
          ) : compareMode && changed ? (
            ...
            <>
              ...
              {processing && (
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-sm text-[10px] text-zinc-400">
                  Rendering...
                </div>
              )}
            </>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-700/60">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="text-[10px] px-1.5 py-1 rounded-md border border-zinc-700/60 text-zinc-500 hover:text-zinc-300 disabled:opacity-30 disabled:cursor-default transition cursor-pointer"
              title="Undo (Ctrl+Z)"
            >
              &#8617;
            </button>
            ...
            <button
              ...
              disabled={!changed}
              className={cn(
                'text-[10px] px-2 py-1 rounded-md border transition cursor-pointer select-none',
                holdOriginal
            ...
            <button
              ...
              disabled={!changed}
              className={cn(
                'text-[10px] px-2 py-1 rounded-md border transition cursor-pointer',
                compareMode
          ...
          <div className="flex flex-wrap gap-1">
            {PRESETS.map((p) => (
              <button
                ...
                className={cn(
                  'text-[10px] px-2 py-0.5 rounded-full border transition cursor-pointer',
                  activePreset === p.name
            ...
            <div className="flex flex-wrap gap-1">
              {customPresets.map((p) => (
                <span key={p.name} className={cn(
                  'inline-flex items-center gap-0.5 text-[10px] px-2 py-0.5 rounded-full border transition',
                  activePreset === p.name
                    ? 'border-purple-400 bg-purple-500/25 text-purple-200 ring-1 ring-purple-500/30'
                    : 'border-purple-500/40 bg-purple-500/10 text-purple-300',
                )}>
                  <button onClick={() => applyPreset(p)} className="cursor-pointer hover:text-purple-200">{p.name}</button>
                  <button onClick={() => handleDeletePreset(p.name)} className="text-purple-500 hover:text-red-400 cursor-pointer ml-0.5">&times;</button>
                </span>
              ))}
            </div>
          )}
            ...
            {showSavePreset ? (
              <>
                <input
                  ...
                  className="flex-1 h-5 bg-zinc-800 border border-zinc-700/60 rounded px-1.5 text-[10px] text-zinc-200 outline-none focus:border-purple-500/60"
                  autoFocus
                />
                <button onClick={handleSavePreset} disabled={!presetName.trim() || !changed} className="text-[10px] text-purple-400 hover:text-purple-300 cursor-pointer disabled:text-zinc-600">Save</button>
                <button onClick={() => { setShowSavePreset(false); setPresetName(''); }} className="text-[10px] text-zinc-500 hover:text-zinc-300 cursor-pointer">&times;</button>
              </>
            ) : (
              <button
                onClick={() => setShowSavePreset(true)}
                disabled={!changed}
                className="text-[10px] text-zinc-500 hover:text-purple-400 cursor-pointer disabled:text-zinc-700 transition"
              >
                + Save as preset
              </button>
            )}
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      className="w-8 text-right text-[10px] font-mono text-zinc-500 bg-transparent border-none outline-none focus:text-zinc-200 p-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-full accent-blue-500 h-1 rounded-full appearance-none bg-zinc-700 cursor-pointer"
                    />
                    <button
                      onClick={() => setValue(a.key, a.default)}
                      className={cn(
                        'text-[9px] px-1 rounded cursor-pointer transition',
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            values.rgbSplitColor === c.id
                              ? 'border-white ring-1 ring-blue-500/50 scale-110'
