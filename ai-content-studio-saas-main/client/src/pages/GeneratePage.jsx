...
export default function GeneratePage() {
  ...
  const resultImageSrc = result?.galleryId || result?.imageId
    ? `/api/gallery/${result.galleryId || result.imageId}/image`
    : null;
  const resultLightboxSrc = result?.image?.base64Data
      ...
      <div className="lg:hidden space-y-4">
        ...
        {result && (
          <Card className="animate-in !p-3">
            <ImageCard src={resultImageSrc} base64={result.image?.base64Data} mimeType={result.image?.mimeType}
              meta={{ imageId: result.imageId, identityConfidence: result.image?.validation?.identity_match_score }}
              onSelect={() => resultLightboxSrc && openLightbox([resultLightboxSrc], 0)} />
          </Card>
        )}
      </div>
