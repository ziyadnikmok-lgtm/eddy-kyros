...
export default function PostClonePage() {
  ...
  const [platform, setPlatform] = useState(_cache.platform);
  ...
  useEffect(() => { _cache.platform = platform; }, [platform]);
        ...
        <div className="lg:col-span-1 space-y-4">
          <Card className="space-y-4">
            <div className="space-y-2">
              <span className="text-xs text-zinc-400 font-medium block">Platform</span>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => { setPlatform('instagram'); setPostUrl(''); setProfileUrl(''); }}
                  className={`flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition cursor-pointer ${platform === 'instagram' ? 'bg-gradient-to-r from-pink-600/80 to-purple-600/80 text-white border border-pink-500/40' : 'bg-zinc-800/60 text-zinc-400 border border-zinc-700/60 hover:bg-zinc-700/60'}`}
                >
                ...
                <button
                  type="button"
                  onClick={() => { setPlatform('pinterest'); setPostUrl(''); setProfileUrl(''); }}
                  className={`flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition cursor-pointer ${platform === 'pinterest' ? 'bg-gradient-to-r from-red-600/80 to-red-700/80 text-white border border-red-500/40' : 'bg-zinc-800/60 text-zinc-400 border border-zinc-700/60 hover:bg-zinc-700/60'}`}
                >
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Btn type="button" variant={inputMode === 'single' ? 'primary' : 'secondary'} onClick={() => { setInputMode('single'); handleBack(); }}>{platform === 'pinterest' ? 'Single Pin' : 'Single/Carousel'}</Btn>
                <Btn type="button" variant={inputMode === 'profile' ? 'primary' : 'secondary'} onClick={() => { setInputMode('profile'); postCloneStore.setValue('result', []); }}>{platform === 'pinterest' ? 'Board Scrape' : 'Profile Scrape'}</Btn>
              </div>
            </div>
            ...
            {inputMode === 'single' ? (
              <>
                <Input
                  label={platform === 'pinterest' ? 'Pinterest Pin URL' : 'Instagram Post URL'}
                  placeholder={platform === 'pinterest' ? 'https://www.pinterest.com/pin/...' : 'https://www.instagram.com/p/...'}
                  value={postUrl}
              ...
              <>
                <Input
                  label={platform === 'pinterest' ? 'Pinterest Board URL' : 'Instagram Profile URL'}
                  placeholder={platform === 'pinterest' ? 'https://www.pinterest.com/username/board-name/' : 'https://www.instagram.com/username/'}
                  value={profileUrl}
          ...
          {platform === 'pinterest' && (
            <Card className="space-y-2">
              <p className="text-xs text-zinc-400 leading-relaxed">
                <span className="font-semibold text-red-400">Pinterest</span> — paste any public pin or board URL. No session needed. Requires your Apify key.
              </p>
            </Card>
          )}

          {platform === 'instagram' && <Card className="!p-0 overflow-hidden">
            <button
              type="button"
