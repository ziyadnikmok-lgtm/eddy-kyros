...
class ApiKeyManager {
  ...
  setMuapiKey(apiKey) {
    if (!apiKey || typeof apiKey !== 'string' || apiKey.trim().length < 10) {
      throw new AppError('A valid Muapi API key is required (min 10 chars)', 400, 'VALIDATION_ERROR');
    }
    const value = apiKey.trim();
    this._store.muapiKeyEncrypted = this._encrypt(value);
    this._store.muapiKeyMasked = this._maskKey(value);
    this._store.muapiUpdatedAt = new Date().toISOString();
    this._saveStore();
    return {
      hasMuapiKey: true,
      maskedKey: this._store.muapiKeyMasked,
      updatedAt: this._store.muapiUpdatedAt,
    };
  }

  getMuapiKey() {
    if (!this._store.muapiKeyEncrypted) return '';
    return this._decrypt(this._store.muapiKeyEncrypted);
  }

  getMuapiKeyInfo() {
    return {
      hasMuapiKey: !!this._store.muapiKeyEncrypted,
      maskedKey: this._store.muapiKeyMasked || '',
      updatedAt: this._store.muapiUpdatedAt || null,
    };
  }

  clearMuapiKey() {
    this._store.muapiKeyEncrypted = null;
    this._store.muapiKeyMasked = '';
    this._store.muapiUpdatedAt = null;
    this._saveStore();
    return { removed: true };
  }
      ...
      if (fs.existsSync(dataFile)) {
        ...
        if (Array.isArray(parsed.keys)) {
          return {
            ...
            muapiKeyEncrypted: parsed.muapiKeyEncrypted || null,
            muapiKeyMasked: parsed.muapiKeyMasked || '',
            muapiUpdatedAt: parsed.muapiUpdatedAt || null,
