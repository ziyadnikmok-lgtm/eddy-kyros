...
class ReferenceManager {
  ...
  // Reorder primary images. `order` is the new sequence of the CURRENT indices, e.g. [2,0,1]
  // means "the image now at index 2 becomes first". Index 0 is the face anchor (prompt image 1).
  reorderPrimaryImages(characterId, order) {
    const { data, charDir } = this._findCharacterById(characterId);
    this._migratePrimaryImages(data);

    const n = data.primaryImageFiles.length;
    const valid = Array.isArray(order)
      && order.length === n
      && order.every((i) => Number.isInteger(i) && i >= 0 && i < n)
      && new Set(order).size === n;
    if (!valid) {
      throw new AppError('Invalid order — must be a permutation of the current image indices', 400, 'VALIDATION_ERROR');
    }

    data.primaryImageFiles = order.map((i) => data.primaryImageFiles[i]);
    data.primaryImageFile = data.primaryImageFiles[0];
    this._writeCharacterJson(charDir, data);
  ...
  _findCharacterById(characterId) {
    if (_characterCache.has(characterId)) {
      ...
      const jsonPath = path.join(cached.charDir, CHARACTER_JSON);
      if (fs.existsSync(jsonPath)) {
        // Re-read from disk if file was modified since we cached it
        try {
          const stat = fs.statSync(jsonPath);
          if (!cached.mtime || stat.mtimeMs !== cached.mtime) {
            const freshData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
            const updated = { data: freshData, charDir: cached.charDir, mtime: stat.mtimeMs };
            _characterCache.set(characterId, updated);
            return updated;
          }
    ...
    for (const entry of entries) {
      ...
      try {
        const data = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
        const stat = fs.statSync(jsonPath);
        _characterCache.set(data.id, { data, charDir, mtime: stat.mtimeMs });
