    ...
    try {
      ...
      const config = {
        ...
        imageConfig: {
          ...
          // NOTE: safetySetting is Imagen-only — do NOT add it here for Gemini Flash
        },
      };
...

{
  ...
  // Video analysis on Vertex. Vertex has no Files API like the direct Gemini API, so the clip
  // goes inline as base64 — fine for the short reference clips this app uses (Muapi caps them
  // at 15s), but a very large file will exceed the request limit and is rejected up front.
  async analyzeVideoWithPrompt(apiKey, videoBase64, mimeType, prompt, refImage = null) {
    this._checkBudget();
    if (!videoBase64 || !mimeType) throw new AppError('Video data and mime type required', 400, 'VALIDATION_ERROR');
    if (!prompt || typeof prompt !== 'string' || !prompt.trim()) throw new AppError('Analysis prompt is required', 400, 'VALIDATION_ERROR');
    // base64 is ~4/3 of the raw bytes; Vertex rejects oversized inline payloads.
    if (videoBase64.length > 26_000_000) {
      throw new AppError('Clip is too large to analyse on Vertex (~20MB max). Trim it or use a shorter reference.', 413, 'PAYLOAD_TOO_LARGE');
    }
    try {
      const genAI = await getClient();
      const response = await withTimeout(
        genAI.models.generateContent({
          ...
          contents: [{ role: 'user', parts: [
            ...
            ...(refImage?.base64 ? [{ inlineData: { mimeType: refImage.mimeType || 'image/png', data: refImage.base64 } }] : []),
            { text: prompt.trim() },
          ] }],
          config: { responseModalities: [Modality.TEXT] },
        }),
        180_000, 'Vertex video analysis'
      );
      const parts = response.candidates?.[0]?.content?.parts;
      const text = (parts || []).filter((x) => x.text).map((x) => x.text).join('').trim();
      if (!text) throw new AppError('Vertex returned no analysis for this clip', 502, 'GENERATION_EMPTY');
      this._trackTextSpend(response);
      return text;
    } catch (err) {
      if (err instanceof AppError) throw err;
...
Return ONLY valid JSON: {"score": <number 0-100>, "reasons": ["<reason1>", "<reason2>", "<reason3>"]}`;
  ...
  _handleApiError(err) {
    ...
    if (message.includes('429') || message.includes('RESOURCE_EXHAUSTED')) {
      throw new AppError('Vertex AI rate limit reached (GCP quota). Wait and try again.', 429, 'RATE_LIMITED');
    }
