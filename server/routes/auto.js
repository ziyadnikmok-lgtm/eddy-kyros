...
router.post('/plans/:id/execute-day', async (req, res, next) => {
  ...
  try {
    ...
    logUsageEvent({
      ...
      entityId: runId,
      source: 'auto-day',
      payload: { feature: 'auto-day', dayNumber, imageCount: entries.length },
    });
...
router.post('/execute', async (req, res, next) => {
  ...
  try {
    ...
    const autoRunId = startGenerationRun({
      ...
      provider: 'gemini',
      model: result.resolvedImageModel || null,
    });
    finishGenerationRun(autoRunId, {
      status: 'succeeded',
      outputCount: data.totalImages,
      provider: 'gemini',
      model: result.resolvedImageModel || null,
    });
    logUsageEvent({
      ...
      entityId: autoRunId,
      source: 'auto-execute',
      payload: { feature: 'auto-execute', totalImages: data.totalImages },
    });
