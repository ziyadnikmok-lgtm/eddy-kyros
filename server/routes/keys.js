...
router.get('/muapi', (_req, res, next) => {
  try {
    const data = apiKeyManager.getMuapiKeyInfo();
    res.json({ success: true, data });
  } catch (err) {
    next(err);
  }
...
router.put('/muapi', (req, res, next) => {
  try {
    const { apiKey } = req.body || {};
    if (!apiKey || typeof apiKey !== 'string' || apiKey.trim().length === 0) {
      throw new AppError('"apiKey" is required and must be a non-empty string', 400, 'VALIDATION_ERROR');
    }
    const data = apiKeyManager.setMuapiKey(apiKey);
    invalidateHealthCache();
    res.json({ success: true, data });
  } catch (err) {
    next(err);
...
router.delete('/muapi', (_req, res, next) => {
  try {
    const data = apiKeyManager.clearMuapiKey();
    invalidateHealthCache();
    res.json({ success: true, data });
  } catch (err) {
    next(err);
...
function checkMuapiHealth() {
  const info = apiKeyManager.getMuapiKeyInfo();
  return {
    configured: !!info.hasMuapiKey,
    live: !!info.hasMuapiKey,
    status: info.hasMuapiKey ? 'ok' : 'missing',
    latencyMs: 0,
    message: info.hasMuapiKey ? `Key stored (${info.maskedKey})` : 'No Muapi key stored',
  };
}
