...
// Reorder primary images (drag-to-reorder). Body: { order: [newSeqOfCurrentIndices] }.
router.put('/:id/primary-images/order', (req, res, next) => {
  try {
    const { order } = req.body || {};
    const character = referenceManager.reorderPrimaryImages(req.params.id, order);
    res.json({ success: true, data: character });
  } catch (err) {
    next(err);
  }
