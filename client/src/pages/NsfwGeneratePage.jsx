  ...
  return (
    <Card className="!p-0 overflow-hidden">
      <div className="relative border-b border-zinc-800/70 bg-zinc-950/80 p-4">
        ...
        <div className="relative space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[0.625rem] font-mono text-zinc-500">{job.aspectRatio}</span>
          </div>
      ...
      <div className="space-y-2 p-3">
        ...
        {job.status === 'running' ? (
          <div className="text-[0.6875rem] text-zinc-500">You can leave this page and come back while it is still running.</div>
        ) : (
...
export default function NsfwGeneratePage() {
  ...
  const downloadImg = async (img, prefix = 'wavespeed') => {
    const blob = await (await fetch(`data:${img.mimeType};base64,${img.base64Data}`)).blob();
    await downloadBlob(blob, `${prefix}_${Date.now()}.png`);
  };
      ...
      <div className="space-y-4 max-w-sm pb-8">
        <Card className="p-4 space-y-4">
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2 py-0.5 text-[0.625rem] font-medium text-amber-200">
            ...
            <p className="text-[0.6875rem] leading-relaxed text-amber-200/70">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => setShowSavePreset(!showSavePreset)} className="text-xs text-rose-400 hover:text-rose-300 cursor-pointer">
            ...
            {selectedPreset && (
              <div className="bg-rose-900/20 rounded-lg px-3 py-2 border border-rose-800/30 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="min-w-0 flex-1">
                    <span className="text-xs text-rose-300 font-medium block truncate">{selectedPreset.name}</span>
                    <span className="text-[0.625rem] text-zinc-500 block truncate">{selectedPreset.path}</span>
                    {selectedPreset.triggerPrompt && (
                      <span className="text-[0.625rem] text-rose-400/70 block truncate mt-0.5">Trigger: {selectedPreset.triggerPrompt}</span>
                    )}
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-0.5">
                    <span className="text-[0.625rem] text-zinc-400">Strength</span>
                    <span className="text-[0.625rem] text-zinc-500 font-mono">{presetStrength.toFixed(1)}</span>
                  </div>
                  <input type="range" min={0.1} max={2.0} step={0.1} value={presetStrength} onChange={(e) => { const v = parseFloat(e.target.value); setPresetStrength(v); sync('presetStrength', v); }} className="w-full accent-rose-500" />
                </div>
              </div>
            )}
            {showSavePreset && (
              <div className="space-y-2 bg-zinc-800/40 rounded-lg p-3 border border-zinc-700/40">
                <input type="text" value={newPresetName} onChange={(e) => setNewPresetName(e.target.value)} placeholder="Preset name (e.g. Maria v2)" className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-rose-500/70 placeholder:text-zinc-600" />
                <input type="text" value={newPresetPath} onChange={(e) => setNewPresetPath(e.target.value)} placeholder="HuggingFace URL or model path" className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-rose-500/70 placeholder:text-zinc-600" />
                <textarea value={newPresetTrigger} onChange={(e) => setNewPresetTrigger(e.target.value)} placeholder="Trigger prompt (auto-pastes into prompt when selected)" rows={2} className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-rose-500/70 placeholder:text-zinc-600 resize-none" />
                <div className="flex gap-2 items-center">
                  <div className="flex-1">
                    <input type="number" value={newPresetScale} onChange={(e) => setNewPresetScale(parseFloat(e.target.value) || 0)} min={0} max={4} step={0.1} className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-2 py-2 text-xs text-zinc-100 text-center outline-none focus:border-rose-500/70" />
                    <span className="text-[0.625rem] text-zinc-500 block text-center mt-0.5">scale</span>
                  </div>
                  <Btn onClick={handleSavePreset} className="bg-rose-600 hover:bg-rose-500 text-xs px-3 py-2">Save</Btn>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {extraLoras.length < 2 && (
                <button type="button" onClick={addExtra} className="text-xs text-rose-400 hover:text-rose-300 cursor-pointer">+ Add LoRA</button>
              )}
            </div>
            {extraLoras.map((lora, idx) => (
              <div key={idx} className="flex gap-2 items-start">
                <div className="flex-1">
                  <input type="text" value={lora.path} onChange={(e) => updateExtra(idx, 'path', e.target.value)} placeholder="HuggingFace URL or model path" className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-rose-500/70 focus:ring-1 focus:ring-rose-500/20 placeholder:text-zinc-600" />
                </div>
                <div className="w-20">
                  <input type="number" value={lora.scale} onChange={(e) => updateExtra(idx, 'scale', parseFloat(e.target.value) || 0)} min={0} max={4} step={0.1} className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-2 py-2 text-xs text-zinc-100 text-center outline-none focus:border-rose-500/70" />
                  <span className="text-[0.625rem] text-zinc-500 block text-center mt-0.5">scale</span>
                </div>
            ...
            {extraLoras.length === 0 && <p className="text-[0.625rem] text-zinc-500">Add style or pose LoRAs on top of your character</p>}
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {ASPECT_RATIOS.map((ar) => (
                <button key={ar} onClick={() => { setAspectRatio(ar); sync('aspectRatio', ar); }} className={`rounded-md px-2.5 py-1.5 text-xs font-medium transition cursor-pointer ${aspectRatio === ar ? 'bg-rose-600 text-white' : 'bg-zinc-700/60 text-zinc-400 hover:bg-zinc-600 hover:text-zinc-200'}`}>{ar}</button>
              ))}
            </div>
          </div>

          {/* Generate button */}
          <Btn onClick={handleGenerate} disabled={!prompt.trim()} className="w-full bg-rose-600 hover:bg-rose-500 disabled:opacity-40">
            {activeQueueCount > 0 ? `Queue Another · ${activeQueueCount} running` : 'Generate (WaveSpeed)'}
          </Btn>
          <p className="text-[0.625rem] text-zinc-500 text-center">$0.01 per image &middot; No safety filters &middot; Sub-second latency</p>
        </Card>
        ...
        {result?.image && (
          <Card className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-rose-400">Create Variations</span>
              <span className="text-[0.625rem] text-zinc-500">{variations.length} created</span>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="range" min={0.1} max={1.0} step={0.05} value={varyStrength} onChange={(e) => setVaryStrength(parseFloat(e.target.value))} className="w-full accent-rose-500" />
              <div className="flex justify-between text-[0.625rem] text-zinc-600 mt-0.5">
                <span>Subtle (keep pose)</span>
                <span>Major (new pose)</span>
              </div>
            </div>
            <Btn onClick={handleVary} disabled={!result?.image} className="w-full bg-rose-700 hover:bg-rose-600 disabled:opacity-40">
