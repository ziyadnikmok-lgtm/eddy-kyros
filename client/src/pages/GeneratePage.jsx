  ...
  return (
    <Card className="!p-0 overflow-hidden">
      <div className="relative border-b border-zinc-800/70 bg-zinc-950/80" style={{ aspectRatio: toAspectRatioValue(job.aspectRatio) }}>
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-4">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[0.625rem] font-mono text-zinc-500">{job.aspectRatio} · {job.resolutionTier}</span>
          </div>
                ...
                {GENERATE_STEPS.map((step, idx) => (
                  <div
                    ...
                    className={`flex items-center gap-2 text-[0.6875rem] ${
                      idx < stepIndex ? 'text-green-400' : idx === stepIndex ? 'text-rose-300' : 'text-zinc-600'
                    }`}
                  >
      ...
      <div className="space-y-2 p-3">
        ...
        {job.status === 'running' ? (
          <div className="text-[0.6875rem] text-zinc-500">You can keep editing the prompt and queue the next image.</div>
        ) : (
          ...
          <Card className="!p-0 overflow-hidden border border-zinc-800/70 bg-zinc-950/90 shadow-[0_24px_80px_rgba(0,0,0,0.28)]">
            <div className="border-b border-zinc-800/70 bg-[linear-gradient(180deg,rgba(35,38,45,0.96),rgba(18,19,24,0.96))] px-4 py-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-[0.6875rem] font-semibold uppercase tracking-[0.18em] text-zinc-500">Image Generator</div>
                ...
                <div className="flex shrink-0 flex-col items-end gap-1.5">
                  ...
                  <span className="text-[0.625rem] font-mono text-zinc-500">{state.aspectRatio} · {state.resolutionTier}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 p-4 xl:max-h-[calc(100vh-140px)] xl:overflow-y-auto">
                  ...
                  {[['quick', 'Quick'], ['advanced', 'Advanced']].map(([m, label]) => (
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-xs font-medium transition cursor-pointer ${
                        state.formMode === m ? 'bg-rose-600 text-white shadow-[0_10px_24px_rgba(37,99,235,0.35)]' : 'text-zinc-400 hover:text-zinc-200'
                      }`}
                    >
                      {label}
                    </button>
              ...
              <div className="space-y-2">
                <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-zinc-500">Model</span>
                <select
                  value={imageModel}
                  onChange={(e) => update({ imageModel: e.target.value })}
                  className="w-full rounded-xl border border-zinc-700/80 bg-zinc-900/80 px-3 py-3 text-sm text-zinc-100 outline-none focus:border-rose-500/70 focus:ring-1 focus:ring-rose-500/20 cursor-pointer"
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <div className="text-[0.6875rem] font-semibold uppercase tracking-[0.12em]">Style</div>
                ...
                <button
                  ...
                  className="rounded-xl border border-zinc-800/80 bg-zinc-900/70 px-3 py-3 text-left text-zinc-400 transition hover:border-zinc-700 hover:text-zinc-200 cursor-pointer"
                >
                  <div className="text-[0.6875rem] font-semibold uppercase tracking-[0.12em]">Controls</div>
                  <div className="mt-1 text-xs">{state.formMode === 'quick' ? 'Essential' : 'Detailed'}</div>
                </button>
              </div>

              <div className="space-y-2">
                <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-zinc-500">Prompt</span>
                <Textarea
                  label={null}
              ...
              >
                <div className="mb-2.5 flex items-center justify-between gap-2">
                  <div className="text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-zinc-500">Image Input</div>
                  {referenceImages.length > 0 && (
                    <button type="button" onClick={() => update({ referenceImages: [] })} className="text-[0.625rem] text-zinc-500 hover:text-zinc-300 cursor-pointer transition">Clear all</button>
                  )}
                </div>
                <div className="flex flex-wrap gap-2">
                  {referenceImages.map((img) => (
                    <div key={img.id} className="group relative h-[72px] w-[72px] flex-shrink-0 overflow-hidden rounded-xl border border-zinc-700/80 bg-zinc-950">
                      ...
                      <button
                        ...
                        className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-black/75 text-[0.6875rem] text-zinc-300 opacity-0 transition hover:bg-black hover:text-white group-hover:opacity-100 cursor-pointer"
                      >✕</button>
                    </div>
                  ))}
                  <label className="flex h-[72px] w-[72px] flex-shrink-0 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border border-dashed border-zinc-700/70 bg-zinc-900/60 text-zinc-500 transition hover:border-cyan-500/50 hover:text-cyan-400">
                    ...
                    <span className="text-[0.5625rem] font-medium uppercase tracking-wider">Add</span>
                    <input type="file" accept="image/png,image/jpeg,image/webp" multiple className="hidden" onChange={handleReferenceImageUpload} />
                  </label>
                </div>
                {referenceImages.length === 0 && (
                  <p className="mt-2 text-[0.625rem] text-zinc-600">Paste, drag, or click Add — supports unlimited images</p>
                )}
              </div>
              ...
              <div className={`flex items-center justify-between rounded-xl border px-3 py-2.5 ${enhanceEnabled ? 'border-amber-500/30 bg-amber-500/[0.04]' : 'border-zinc-800/70 bg-zinc-900/60'}`}>
                ...
                <span className="text-[0.625rem] text-zinc-500">{enhanceEnabled ? <span className="text-amber-400/80">2× API calls</span> : 'Off'}</span>
              </div>

              <div className="space-y-2.5 rounded-2xl border border-zinc-800/80 bg-zinc-900/45 p-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-zinc-500">Character</div>
                  {selectedCharId && (
                    <button
                      type="button"
                      onClick={() => update({ useCharacter: false, selectedCharId: '', selectedChar: null })}
                      className="text-[0.625rem] text-zinc-500 hover:text-zinc-300 cursor-pointer transition"
                    >
                      Clear
                    </button>
                  )}
                ...
                {chars.length === 0 ? (
                  <p className="text-[0.6875rem] text-zinc-600 py-1">No characters yet — create one in Characters.</p>
                ) : (
                      ...
                      return (
                        <button
                          ...
                          className={`group flex-shrink-0 flex flex-col items-center gap-1.5 rounded-xl border p-2 transition cursor-pointer ${
                            isSelected
                              ? 'border-rose-500/60 bg-rose-500/10 shadow-[0_0_12px_rgba(59,130,246,0.15)]'
                              : 'border-zinc-800/70 bg-zinc-900/60 hover:border-zinc-600/80 hover:bg-zinc-800/60'
                          }`}
                        >
                          <div className={`relative h-12 w-12 overflow-hidden rounded-lg border transition ${isSelected ? 'border-rose-400/50' : 'border-zinc-700/60 group-hover:border-zinc-600'}`}>
                            <img
                          ...
                          <span className={`max-w-[56px] truncate text-[0.625rem] font-medium transition ${isSelected ? 'text-rose-300' : 'text-zinc-400 group-hover:text-zinc-200'}`}>
                            {c.name}
                          </span>
                        </button>
                      );
                    ...
                    {characterReferencePreviewItems.map((item, index) => (
                      ...
                      >
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-1 pb-0.5 pt-3">
                          <span className="block truncate text-[0.5625rem] text-zinc-300">{item.label}</span>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-zinc-500 block mb-2">Aspect Ratio</span>
                  <div className="flex flex-wrap gap-1.5">
                    {ASPECT_RATIOS.map((ar) => (
                      <button
                        key={ar}
                        onClick={() => update({ aspectRatio: ar })}
                        className={`rounded-lg px-2.5 py-1.5 text-xs font-medium transition cursor-pointer ${aspectRatio === ar ? 'bg-rose-600 text-white shadow-[0_10px_24px_rgba(37,99,235,0.35)]' : 'bg-zinc-800/70 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'}`}
                      >
                        {ar}
                      </button>
                    ))}
                ...
                <div>
                  <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-zinc-500 block mb-2">Resolution</span>
                  <div className="flex flex-wrap gap-1.5">
                    {RESOLUTION_TIERS.map((tier) => (
                      <button
                        key={tier}
                        onClick={() => update({ resolutionTier: tier })}
                        className={`rounded-lg px-3 py-1.5 text-xs font-medium transition cursor-pointer ${resolutionTier === tier ? 'bg-rose-500 text-white shadow-[0_10px_24px_rgba(37,99,235,0.35)]' : 'bg-zinc-800/70 text-zinc-300 hover:bg-zinc-700'}`}
                      >
                        {tier}
                      </button>
                    ))}
              ...
              {state.formMode === 'quick' && (
                <div className="space-y-2">
                  <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-zinc-500 block">Content Type</span>
                  <div className="grid grid-cols-2 gap-2">
                    {CONTENT_TYPES.map(ct => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className={`text-xs font-medium ${state.quickContentType === ct.key ? 'text-rose-300' : 'text-zinc-300'}`}>{ct.label}</span>
                          <span className="text-[0.625rem] text-zinc-600">{ct.pct}%</span>
                        </div>
                        <span className="mt-1 block text-[0.625rem] leading-relaxed text-zinc-500">{ct.desc}</span>
                      </button>
                    ))}
                  </div>
                  {state.quickContentType && (
                    <div className="rounded-xl border border-zinc-800/70 bg-zinc-900/50 px-3 py-2 text-[0.6875rem] text-zinc-500">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {AUTHENTICITY_MODIFIERS.map(mod => (
                    <button key={mod.id} onClick={() => setActiveMods(prev => {
                      ...
                      className={`rounded-full px-2.5 py-1 text-[0.6875rem] font-medium transition cursor-pointer border ${
              ...
              <Section title="Image References" badge={activeRefCount > 0 ? <Badge color="blue">{activeRefCount}</Badge> : null} hint="Upload reference images for outfit, items, or background. The AI will incorporate these into the generation.">
                <div className="grid grid-cols-1 gap-2">
                  <label className={`flex items-center justify-between rounded-lg cursor-pointer transition h-16 overflow-hidden px-3 border border-dashed ${specificOutfitRef ? 'border-rose-500/40 bg-rose-500/5' : 'border-zinc-700/80 bg-zinc-900/50 hover:border-rose-500/30'}`}>
              ...
              <Section title="Style Library" badge={styleAtomIds.length > 0 ? <Badge color="blue">{styleAtomIds.length}</Badge> : null} hint="Reusable style building blocks (lighting, vibe, camera, etc.) that combine into a cohesive visual style.">
                <div className="flex items-center justify-end">
                  <button onClick={() => setShowAtomPicker(true)} className="text-xs text-rose-400 hover:text-rose-300 cursor-pointer">
                    Browse Library
                  </button>
                </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex flex-wrap gap-1">
                      {styleAtomDetails.map(a => (
                        <span key={a.id} className="inline-flex items-center gap-1 bg-rose-500/15 text-rose-400 text-xs px-2 py-0.5 rounded-full">
                          {a.category}
                    ...
                    {stylePreview && (
                      <div className="bg-zinc-800/60 rounded-lg p-2 text-[0.6875rem] text-zinc-400 font-mono whitespace-pre-wrap max-h-20 overflow-y-auto">
                        {stylePreview}
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5">
                      <div className="grid grid-cols-2 gap-1">
                        {Object.entries(selectedFocusData.attributes || {}).filter(([, v]) => v).map(([key, val]) => (
                          <div key={key} className="bg-rose-500/10 border border-rose-500/20 rounded-md px-2 py-1">
                            <div className="text-[0.5625rem] text-rose-400 uppercase tracking-wider">{key}</div>
                            <div className="text-[0.6875rem] text-zinc-300 line-clamp-1">{val}</div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {CONTENT_TYPES.map(ct => (
                      <button key={ct.key} onClick={() => setContentTab(ct.key)}
                        ...
                        <div className="flex items-center justify-between">
                          <span className={`text-xs font-medium ${contentTab === ct.key ? 'text-rose-300' : 'text-zinc-300'}`}>{ct.label}</span>
                          <span className="text-[0.625rem] text-zinc-600">{ct.pct}%</span>
                        </div>
                        <span className="text-[0.625rem] text-zinc-500 block mt-0.5">{ct.desc}</span>
                      </button>
                    ))}
                  </div>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {contentPresets.filter(p => p.contentType === contentTab).map(preset => (
                      <button key={preset.id}
                        ...
                        <span className="text-[0.625rem] text-zinc-500">{preset.description}</span>
                      </button>
                    ))}
                    {contentPresets.filter(p => p.contentType === contentTab).length === 0 && (
                      <p className="text-[0.625rem] text-zinc-600 italic py-1">No presets available</p>
                    )}
                  </div>
                </div>
              </Section>
              ...
              <Section title="Captions" badge={captionList.length > 0 ? <Badge color="zinc">{captionList.length}</Badge> : null}
                ...
                <div className="space-y-2">
                  {suggestedCaptions.length > 0 && (
                    <div className="space-y-1.5">
                      <span className="text-[0.625rem] text-zinc-500 uppercase tracking-wider">Suggested</span>
                      {suggestedCaptions.map(c => (
                        <div key={c.id} className="bg-zinc-800/40 rounded-lg p-2 group">
                          ...
                          <div className="text-[0.6875rem] text-zinc-400 mt-0.5 line-clamp-2">{c.body}</div>
                          {c.hashtags.length > 0 && (
                            <div className="text-[0.625rem] text-rose-400 mt-1 truncate">{c.hashtags.map(h => `#${h}`).join(' ')}</div>
                          )}
                          <div className="flex gap-1.5 mt-1.5 opacity-0 group-hover:opacity-100 transition">
                            ...
                            <button onClick={() => { captionApi.markUsed(c.id).catch(() => {}); notify('Marked as used', 'info'); }}
                              className="text-[0.625rem] text-zinc-500 hover:text-green-400 cursor-pointer">Use</button>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {showCaptionComposer && (
                    <div className="space-y-2 bg-zinc-800/30 rounded-lg p-2.5">
                      <input type="text" placeholder="Caption title..."
                        ...
                        className="w-full h-8 rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 text-sm text-zinc-200 placeholder:text-zinc-500 outline-none focus:border-rose-500/70" />
                      ...
                      <textarea placeholder="Caption body... Use {{placeholder}} for variables" rows={3}
                        ...
                        className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-sm text-zinc-200 placeholder:text-zinc-500 outline-none resize-y focus:border-rose-500/70" />
                      ...
                      {captionList.map(c => (
                        <div key={c.id} className="flex items-center justify-between rounded-lg bg-zinc-800/60 px-2.5 py-1.5 group">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-[0.625rem] text-zinc-500">{c.category}</div>
                          </div>
              ...
              <Section title="Templates" badge={tplList.length > 0 ? <Badge color="zinc">{tplList.length}</Badge> : null}>
                <div className="space-y-2">
                  <div className="flex items-center justify-end">
                    <button onClick={() => setShowSaveTpl(!showSaveTpl)} className="text-xs text-rose-400 hover:text-rose-300 cursor-pointer">
                  ...
                  {showSaveTpl && (
                    <div className="flex gap-2">
                      <input type="text" value={tplName} onChange={(e) => setTplName(e.target.value)} placeholder="Template name..."
                        className="flex-1 h-8 rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 text-sm text-zinc-200 placeholder:text-zinc-500 outline-none transition-all duration-200 hover:border-zinc-600 focus:border-rose-500/70 focus:ring-2 focus:ring-rose-500/20"
                    ...
                    <div className="space-y-1 max-h-32 overflow-y-auto">
                      {tplList.map((t) => (
                        <div key={t.id} className="flex items-center justify-between rounded-lg bg-zinc-800/60 px-2.5 py-1.5 group">
                          <button onClick={() => handleLoadTemplate(t)} className="text-sm text-zinc-300 hover:text-rose-400 transition truncate text-left flex-1 cursor-pointer">{t.name}</button>
              ...
              <div className="pt-1">
                <div className="flex items-center gap-1 mb-2">
                  {promptCompleteness.map(item => (
                    <div key={item.label} className="flex-1 group relative">
                      ...
                      <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[0.5625rem] text-zinc-500 opacity-0 group-hover:opacity-100 transition whitespace-nowrap pointer-events-none">
                ...
                {activeQueueCount > 0 && (
                  <p className="mt-2 text-[0.6875rem] text-zinc-500">Keep generating while earlier images are still processing.</p>
                )}
                {enhancedPreview?.changed && (
                  <div className="mt-2 rounded-xl border border-rose-500/20 bg-rose-500/5 p-2.5 text-xs space-y-1.5">
