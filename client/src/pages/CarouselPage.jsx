          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            <div className={`h-12 w-12 rounded-2xl border ${isError ? 'border-red-500/30 bg-red-500/10' : 'border-zinc-700/60 bg-zinc-800/70'} flex items-center justify-center`}>
              {isError ? (
                ...
                <div className="h-5 w-5 rounded-full border-2 border-rose-500/30 border-t-rose-400 animate-spin" />
              )}
            </div>
            <div className="w-24 h-2 rounded-full bg-zinc-800/80 overflow-hidden">
              {!isError ? <div className="h-full w-1/2 bg-rose-500/50 animate-pulse" /> : null}
            </div>
          </div>
        )}
        ...
        {CAROUSEL_MODES.map(m => (
          <button key={m.key} onClick={() => setCarouselMode(m.key)}
            className={`rounded-md px-4 py-1.5 text-xs font-medium transition cursor-pointer ${
              carouselMode === m.key ? 'bg-rose-600 text-white' : 'text-zinc-400 hover:text-zinc-200'
            }`}>
            {m.label}
          </button>
        ))}
      ...
      <Card className="space-y-5">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[0.625rem] text-zinc-500">Ctrl+V to paste from clipboard</span>
          </div>
          {loadingGallery ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 auto-rows-[120px] gap-2 max-h-[40vh] overflow-y-auto pr-1">
              <label className="group relative flex w-full h-full cursor-pointer items-center justify-center rounded-xl border border-dashed border-zinc-700/80 bg-zinc-900/70 text-zinc-400 transition hover:border-rose-500/70 hover:bg-zinc-800/80 hover:text-zinc-200">
                ...
                <div className="text-center">
                  ...
                  <div className="mt-1 text-[0.625rem]">Upload / Paste</div>
                </div>
              </label>
          ...
          {selectedImageSrc && (
            <div className="rounded-lg overflow-hidden border border-zinc-700/40">
              ...
              <div className="px-3 py-2 bg-zinc-900/50">
                <p className="text-[0.6875rem] text-zinc-500 line-clamp-2">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <label className="text-xs text-zinc-400">
              ...
              <select
                ...
                className="mt-1 w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-rose-500"
              >
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition cursor-pointer border ${followUpDirection === prompt ? 'bg-rose-600 border-rose-500 text-white' : 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:bg-zinc-700 hover:text-zinc-100'}`}
                >
                  {label}
                </button>
              ))}
            ...
            <input
              ...
              className="w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-rose-500"
            />
          </div>
          ...
          {followUpLoading && (
            <div className="text-[0.6875rem] text-zinc-500">{followUpElapsedSec}s elapsed</div>
          )}
        </div>
      </Card>
      )}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {RESOLUTION_TIERS.map(tier => (
                    <button key={tier} onClick={() => setResolutionTier(tier)}
                      className={`rounded-md px-2 py-1 text-xs font-medium transition cursor-pointer ${resolutionTier === tier ? 'bg-rose-500 text-white' : 'bg-zinc-700/60 text-zinc-200 hover:bg-zinc-600'}`}>
                      {tier}
                    </button>
                  ))}
                </div>
          ...
          <Card className="space-y-3">
            ...
            <label className="text-xs text-zinc-400 block">
              ...
              <input value={pollTopic} onChange={(e) => setPollTopic(e.target.value)}
                ...
                className="mt-1 w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2.5 text-sm text-zinc-100 outline-none focus:border-rose-500/70" />
            </label>
            <label className="text-xs text-zinc-400 block">
              Number of Questions
              <select value={pollCount} onChange={(e) => setPollCount(Number(e.target.value))}
                className="mt-1 w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-rose-500/70">
                ...
                <div key={poll.question || i} className="rounded-lg border border-zinc-700/40 bg-zinc-800/40 p-2.5 space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    <div className="rounded-md bg-rose-500/10 border border-rose-500/20 px-2 py-1">
                      <span className="text-[0.5625rem] text-rose-400 uppercase tracking-wider">A</span>
                      <p className="text-[0.6875rem] text-zinc-300">{poll.optionA?.label}</p>
                    </div>
                    <div className="rounded-md bg-rose-500/10 border border-rose-500/20 px-2 py-1">
                      <span className="text-[0.5625rem] text-rose-400 uppercase tracking-wider">B</span>
                      <p className="text-[0.6875rem] text-zinc-300">{poll.optionB?.label}</p>
                    </div>
                  </div>
                </div>
              ))}
