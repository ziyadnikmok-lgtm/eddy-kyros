        ...
        <div className="flex-1 min-w-0 space-y-4">
            ...
            {CATEGORIES.map(cat => {
              ...
              return (
                ...
                >
                  ...
                  {count > 0 && <span className="ml-1.5 text-[10px] opacity-50">{count}</span>}
                </button>
              );
            })}
          </div>
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            <select
              ...
              className="bg-zinc-900/60 border border-zinc-700/80 rounded-lg px-3 py-2.5 text-sm text-zinc-300 outline-none focus:border-blue-500/70 focus:ring-1 focus:ring-blue-500/20"
            >
            ...
            <div className="sticky top-0 max-h-[calc(100vh-7rem)] flex flex-col rounded-xl border border-zinc-700/80 bg-zinc-900 overflow-hidden">
              ...
              <div className="px-4 pt-4 pb-3 border-b border-zinc-800">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setSelectedIds(new Set())} className="text-[11px] text-zinc-500 hover:text-zinc-300 cursor-pointer">Clear all</button>
                </div>
                ...
                {Object.entries(selectedGrouped).map(([cat, catAtoms]) => (
                  <div key={cat}>
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-[10px] text-zinc-600">{catAtoms.length}</span>
                    </div>
                    <div className="space-y-1">
                      {catAtoms.map(a => (
                        <div key={a.id} className="group flex items-start gap-2 py-1.5 px-2 rounded-lg bg-zinc-800/50 hover:bg-zinc-800/80">
                          <p className="flex-1 text-[11px] text-zinc-300 leading-relaxed line-clamp-2">{a.text}</p>
                          <button
                  ...
                  <div className="pt-2 border-t border-zinc-800">
                    ...
                    <div className="space-y-1.5">
                      {composedPrompt.split('\n').filter(Boolean).map((line, i) => {
                        ...
                        if (colonIdx === -1) return <p key={i} className="text-[11px] text-zinc-400 leading-relaxed">{line}</p>;
                        ...
                        return (
                          <div key={i} className="text-[11px] leading-relaxed">
                            <span className={`font-medium ${
                              CATEGORY_COLORS[cat] === 'blue' ? 'text-blue-400' :
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-1.5">
        ...
        <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button onClick={e => { e.stopPropagation(); onEdit(); }} className="text-lg leading-none text-zinc-600 hover:text-blue-400 cursor-pointer" title="Edit">
            {'\u270E'}
          </button>
      ...
      {atom.tags?.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          {atom.tags.slice(0, 3).map(tag => (
            <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-700/50 text-zinc-500">{humanTag(tag)}</span>
          ))}
          {atom.tags.length > 3 && <span className="text-[10px] text-zinc-600">+{atom.tags.length - 3}</span>}
        </div>
      )}
          ...
          <>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setCheckedAtoms(new Set(importPreview.map((_, i) => i)))} className="text-xs text-blue-400 hover:text-blue-300 cursor-pointer">Select All</button>
                ...
                <div className="space-y-1">
                  {items.map(atom => (
                    <label key={atom._idx} className="flex items-start gap-2 p-2 rounded-lg bg-zinc-800/40 hover:bg-zinc-800/70 cursor-pointer">
                      <input type="checkbox" checked={checkedAtoms.has(atom._idx)} onChange={() => toggleCheck(atom._idx)} className="mt-0.5 accent-blue-500" />
