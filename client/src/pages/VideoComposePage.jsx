    ...
    <div className="flex min-w-0 gap-2">
      ...
      >
        ...
        {file ? (
          <div className="mx-auto flex max-w-full flex-col items-center gap-1">
            ...
            <div className="text-[0.6875rem] text-zinc-500">{(file.size / 1024 / 1024).toFixed(1)} MB</div>
          </div>
        ) : (
          <>
            <div className="text-3xl mb-2 opacity-30">+</div>
            <div className="text-sm text-zinc-400">{label}</div>
            <div className="text-[0.6875rem] text-zinc-600 mt-0.5">Drop, paste, or click to browse</div>
          </>
        )}
      </button>
      {showGalleryPicker && !file && (
        <button type="button" onClick={showGalleryPicker}
          ...
          <span className="text-[0.5625rem] uppercase tracking-wider text-zinc-400">Gallery</span>
        </button>
      )}
    </div>
  );
...
function DraggableTimelineClip({ clip, timelineDuration, isSelected, onSelect, onUpdate }) {
  ...
  return (
    ...
    >
      ...
      />
      <span className="text-[0.5625rem] text-fuchsia-200 truncate flex-1 px-1 pointer-events-none select-none drop-shadow">{String(clip.text || '').replace(/\s*\n\s*/g, ' / ')}</span>
      <div 
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          {hint ? <div className="text-[0.625rem] uppercase tracking-wider text-zinc-600">{hint}</div> : null}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className="rounded-md border border-zinc-700 bg-zinc-900 px-2 py-1 text-[0.6875rem] font-mono text-zinc-300 tabular-nums">
          ...
          <button
            ...
            className="rounded-md border border-zinc-700 px-2 py-1 text-[0.625rem] uppercase tracking-wider text-zinc-500 transition hover:border-zinc-600 hover:text-zinc-300 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            Reset
          </button>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          ...
          className="w-full accent-rose-500 h-1.5 rounded-full appearance-none bg-zinc-700 cursor-pointer"
        />
        <button
          type="button"
          onClick={() => adjustByStep(1)}
    ...
    <div className="flex flex-col bg-[#131313] text-zinc-100 rounded-xl border border-zinc-800/80 overflow-hidden animate-in" style={{ minHeight: 700, height: 'calc(100vh - 130px)' }}>
      ...
      <header className="flex items-center justify-between px-5 py-2.5 border-b border-zinc-800 bg-[#1c1c1c] shrink-0">
        <div>
          ...
          <p className="text-[0.625rem] text-zinc-600">trim · captions · audio · effects</p>
        </div>
        <div className="flex items-center gap-2">
          {videoFile && duration > 0 && (
            <span className="text-[0.6875rem] font-mono text-zinc-500 tabular-nums">{timelineDuration.toFixed(2)}s</span>
          )}
          <span className="rounded-full border border-zinc-700 bg-zinc-900 px-2.5 py-0.5 text-[0.625rem] text-zinc-500">9:16</span>
          ...
          <div className="grid grid-cols-3 gap-1 border-b border-zinc-800 bg-zinc-900/40 p-2 shrink-0">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setPanel(t.id)}
                className={`min-w-0 rounded-lg px-2 py-2 text-[0.5625rem] font-semibold uppercase tracking-[0.12em] transition ${panel === t.id ? 'bg-cyan-500/12 text-cyan-300 shadow-[inset_0_-2px_0_rgba(34,211,238,0.85)]' : 'text-zinc-600 hover:bg-zinc-800/60 hover:text-zinc-300'}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-4">
            ...
            {panel === 'media' && <>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/25 p-3">
                <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Media</div>
                <div className="mt-1 text-[0.6875rem] text-zinc-500">Load a reel clip, trim it, then pick the look you want.</div>
              </div>
              <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600 mb-1">Source Clip</div>
              ...
              <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600 mt-3 mb-1">Append Second Clip</div>
              ...
              {videoFile2 && (
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/20 p-3 text-[0.6875rem] text-zinc-500">
              ...
              {duration > 0 && (
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 space-y-3">
                  <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Trim Range</div>
              ...
              <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600 mt-2 mb-1">Color Preset</div>
            ...
            {panel === 'text' && <>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/25 p-3">
                <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Text Layers</div>
                <div className="mt-1 text-[0.6875rem] text-zinc-500">Create hook text, or paste/drop a video here to copy only its visible text overlay.</div>
              </div>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Import Overlay Text</div>
                    <div className="mt-1 text-[0.6875rem] text-zinc-500">Drop/paste a reference video. Kyros reads the text on screen and creates editable caption blocks.</div>
                  </div>
                ...
                {overlaySourceFile && (
                  <button
                    ...
                    className="text-[0.625rem] text-zinc-600 transition hover:text-zinc-300"
                  >
              ...
              <div className="flex items-center justify-between">
                <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Captions</div>
                <button onClick={addClip} className="rounded-lg border border-fuchsia-500/40 bg-fuchsia-500/10 px-2.5 py-1 text-[0.6875rem] text-fuchsia-300 hover:bg-fuchsia-500/20 transition">+ Add</button>
              </div>
              <div className="space-y-1.5">
                {clips.length === 0 && (
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4 text-center text-[0.6875rem] text-zinc-600">
                ...
                {clips.map(clip => (
                  <button key={clip.id} onClick={() => setSelectedClipId(clip.id)}
                    ...
                    <div className="text-[0.625rem] text-zinc-600 mt-0.5">{stamp(clip.start)} {'->'} {stamp(clip.end)}</div>
                  </button>
                ))}
              </div>
              {selectedClip && (
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Edit</div>
                    <button onClick={removeClip} className="text-[0.625rem] text-zinc-700 hover:text-rose-400 transition">Remove</button>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {POSITIONS.map(pos => (
                      <button key={pos} onClick={() => updateClip({ position: pos })}
                        className={`rounded-lg py-1.5 text-[0.625rem] capitalize transition border ${selectedClip.position === pos ? 'border-fuchsia-400/50 bg-fuchsia-500/15 text-fuchsia-300' : 'border-zinc-800 bg-zinc-900/40 text-zinc-500 hover:border-zinc-700'}`}>
                        {pos}
                      </button>
                    ))}
                  </div>
            ...
            {panel === 'audio' && <>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/25 p-3">
                <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Audio</div>
                <div className="mt-1 text-[0.6875rem] text-zinc-500">Preview background music live, line it up by ear, then mix or replace the source audio.</div>
              </div>
              <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600 mb-1">Music Track</div>
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setReplaceOriginalAudio(v => !v)}
                      className={`rounded-lg py-1.5 text-[0.6875rem] font-medium border transition ${replaceOriginalAudio ? 'border-amber-500/40 bg-amber-500/10 text-amber-300' : 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300'}`}>
                      {replaceOriginalAudio ? 'Replace Original' : 'Mix With Original'}
                    </button>
                    <button
                      type="button"
                      onClick={clearAudio}
                      className="rounded-lg border border-rose-500/35 bg-rose-500/10 py-1.5 text-[0.6875rem] font-medium text-rose-300 transition hover:bg-rose-500/20"
                    >
                      Remove Audio
                    </button>
                  </div>
            ...
            {panel === 'adjust' && (
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 space-y-4">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Adjustments</div>
                    <div className="text-[0.6875rem] text-zinc-500">Fine-tune the look, then reset when you want a clean slate.</div>
                  </div>
                  <button
                    type="button"
                    onClick={resetAdjustments}
                    className="rounded-lg border border-zinc-700 px-3 py-1.5 text-[0.625rem] font-semibold uppercase tracking-wider text-zinc-400 transition hover:border-zinc-600 hover:text-zinc-200"
                  >
                    Reset All
                  </button>
                </div>
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3 space-y-3">
                  <div>
                    <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Main Clip Framing</div>
                    <div className="text-[0.6875rem] text-zinc-500">Zoom in and nudge the shot until the crop feels right.</div>
                  </div>
                ...
                {videoFile2 ? (
                  <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3 space-y-3">
                    <div>
                      <div className="text-[0.625rem] uppercase tracking-widest text-zinc-600">Second Clip Framing</div>
                      <div className="text-[0.6875rem] text-zinc-500">Tune the appended clip so the export feels matched.</div>
                    </div>
            ...
            {panel === 'export' && (
              <div className="space-y-3">
                <div className="rounded-xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 to-zinc-900/60 p-3">
                  <div className="text-[0.625rem] uppercase tracking-widest text-cyan-300/80">Ready To Export</div>
                  <div className="mt-1 text-[0.6875rem] text-zinc-400">Review the reel settings, then save it straight into your gallery.</div>
                </div>
                ...
                {result?.localFilename && (
                  <div className="space-y-3 rounded-xl border border-emerald-500/25 bg-emerald-500/5 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-[0.625rem] uppercase tracking-widest text-emerald-300/80">Saved To Gallery</div>
                        <div className="mt-1 text-[0.6875rem] text-zinc-400">Your reel export is ready to preview, open, or download.</div>
                      </div>
                      <div className="rounded-full border border-emerald-400/25 bg-emerald-500/10 px-2.5 py-1 text-[0.625rem] font-semibold uppercase tracking-wider text-emerald-300">Done</div>
                    </div>
                    <video src={videoApi.fileUrl(result.localFilename)} controls className="w-full rounded-xl border border-zinc-800 bg-black" />
                    <div className="grid grid-cols-2 gap-2">
                      <button onClick={() => navigateTo('videoGallery')} className="rounded-lg border border-zinc-700 py-2 text-[0.6875rem] text-zinc-300 hover:border-zinc-600 transition">Open Gallery</button>
                      <button onClick={() => { const a = document.createElement('a'); a.href = videoApi.cleanFileUrl(result.localFilename); a.download = result.localFilename.replace(/(\.[^.]+)$/, '_metadatacleaned$1'); a.click(); }}
                        className="rounded-lg bg-zinc-700 py-2 text-[0.6875rem] text-zinc-100 hover:bg-zinc-600 transition">Download MP4</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {videoFile && duration > 0 && !primaryIsImage && (
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-5 pb-4 pt-10">
                <div className="flex items-center gap-3">
                  ...
                  <span className="min-w-[96px] text-right text-[0.6875rem] font-mono tabular-nums text-white/90 shrink-0">{stamp(displayTime)} / {stamp(timelineDuration)}</span>
                </div>
              </div>
            )}
          </div>
                ...
                <div className="absolute inset-y-0" style={laneInsetStyle}>
                  {rulerTicks.map((tick, i) => (
                    <div key={i} className="absolute top-0 flex flex-col items-start" style={{ left: `${tick.pct}%` }}>
                      ...
                      {i % 2 === 0 && <span className="text-[0.5625rem] text-zinc-600 mt-0.5 pl-0.5 whitespace-nowrap">{tick.label.slice(3)}</span>}
                    </div>
                  ))}
                </div>
                {/* Playhead needle */}
            ...
            <div className="flex flex-col" style={{ height: 173 }}>
              ...
              <div className="flex border-b border-zinc-800/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.015),rgba(255,255,255,0))]" style={{ height: 64 }}>
                <div className="w-16 shrink-0 border-r border-zinc-800/60 bg-[#161616] flex items-center justify-center">
                  <span className="text-[0.5625rem] font-semibold uppercase tracking-[0.28em] text-zinc-600 [writing-mode:vertical-lr] rotate-180">Video</span>
                </div>
                  ...
                  <div className="absolute inset-y-0" style={laneInsetStyle}>
                    {videoFile && (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center px-2 pointer-events-none">
                          <span className="text-[0.625rem] text-cyan-200/70 font-medium truncate drop-shadow">{videoFile.name}{primaryIsImage ? ` · ${imageDuration.toFixed(1)}s` : ''}</span>
                        </div>
                      </DraggableMediaClip>
                    )}
                    {videoFile2 && secondVisibleDuration > 0 && (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center px-2 pointer-events-none">
                          <span className="text-[0.625rem] text-cyan-100/75 font-medium truncate drop-shadow">{videoFile2.name}{secondaryIsImage ? ` · ${imageDuration2.toFixed(1)}s` : ''}</span>
                        </div>
                      </div>
                    )}
                    {!videoFile && <div className="absolute inset-0 flex items-center px-4 text-[0.625rem] text-zinc-700">Add a source clip to begin</div>}
                  </div>
                  {/* Playhead */}
              ...
              <div className="flex border-b border-zinc-800/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.01),rgba(255,255,255,0))]" style={{ height: 48 }}>
                <div className="w-16 shrink-0 border-r border-zinc-800/60 bg-[#161616] flex items-center justify-center">
                  <span className="text-[0.5625rem] font-semibold uppercase tracking-[0.28em] text-zinc-600 [writing-mode:vertical-lr] rotate-180">Text</span>
                </div>
                <div className="flex-1 relative cursor-pointer bg-zinc-900/20" onClick={seekFromTimeline} data-timeline-track="true">
                  <div className="absolute inset-y-0" style={laneInsetStyle}>
                    {clips.map(clip => (
                    ...
                    {clips.length === 0 && <div className="absolute inset-0 flex items-center px-4 text-[0.625rem] text-zinc-700">Add caption blocks from the Text tab</div>}
                  </div>
                  <div className="absolute inset-0 pointer-events-none z-10">
                    <div className="absolute top-0 bottom-0 w-px bg-cyan-400/70 -translate-x-1/2" style={lanePlayheadStyle} />
                  </div>
              ...
              <div className="flex bg-[linear-gradient(180deg,rgba(255,255,255,0.008),rgba(255,255,255,0))]" style={{ height: 60 }}>
                <div className="w-16 shrink-0 border-r border-zinc-800/60 bg-[#161616] flex items-center justify-center">
                  <span className="text-[0.5625rem] font-semibold uppercase tracking-[0.28em] text-zinc-600 [writing-mode:vertical-lr] rotate-180">Audio</span>
                </div>
                <div className="flex-1 relative cursor-pointer bg-zinc-900/20" onClick={seekFromTimeline} data-timeline-track="true">
                  <div className="absolute inset-y-0" style={laneInsetStyle}>
                    {audioFile ? (
                      ...
                      >
                        ...
                        <span className="relative z-10 text-[0.625rem] text-emerald-300/70 font-medium truncate px-2 pointer-events-none drop-shadow">{audioFile.name}</span>
                      </DraggableMediaClip>
                    ) : (
                      <div className="absolute inset-0 flex items-center px-4 text-[0.625rem] text-zinc-700">Add music from the Audio tab to preview and export it</div>
                    )}
                  </div>
                  <div className="absolute inset-0 pointer-events-none z-10">
                    <div className="absolute top-0 bottom-0 w-px bg-cyan-400/70 -translate-x-1/2" style={lanePlayheadStyle} />
                  ...
                  {pickerItems.map(item => (
                    <button key={item.id} onClick={() => selectGalleryVideo(item)}
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-3 pointer-events-none">
                        <div className="text-[0.625rem] text-zinc-400 capitalize">{item.model || 'Video'}</div>
