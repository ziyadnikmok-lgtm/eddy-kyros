                  ...
                  {images.map(img => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full h-full overflow-hidden rounded-lg border transition duration-150 ${isSelected ? 'border-blue-500 ring-1 ring-blue-500/40' : 'border-zinc-800 hover:border-zinc-600'}`}
                      >
              ...
              <div className="grid grid-cols-2 gap-2">
                {RATIOS.map(r => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center rounded-xl border py-3 px-2 text-center transition cursor-pointer ${targetRatio === r.value ? 'border-blue-500 bg-blue-600/10 text-blue-400' : 'border-zinc-800 bg-zinc-900/50 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200'}`}
                  >
                    <span className="text-lg leading-none mb-1">{r.icon}</span>
                    <span className="text-xs font-semibold">{r.label}</span>
                    <span className="text-[10px] text-zinc-500 mt-0.5">{r.desc}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {result && !running && (
            <Card className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-green-400 bg-green-500/10 border border-green-500/20 rounded px-2 py-0.5">✓ Saved to gallery</span>
              </div>
              <div className="rounded-xl overflow-hidden border border-zinc-800 bg-zinc-950">
                <img
              ...
              <Btn
                ...
                onClick={() => {
                  ...
                  a.href = `data:${result.image?.mimeType || 'image/png'};base64,${result.image?.base64Data}`;
                  a.download = `reformat-${targetRatio.replace(':', 'x')}-${Date.now()}.png`;
                  a.click();
                }}
              >
                Download
