...
function BarCell({ value, max, color = 'bg-rose-500' }) {
          ...
          <div className="grid gap-3 grid-cols-3">
            {[
              ...
              <div key={m.label} className="rounded-lg border border-zinc-800/60 p-3">
                <div className="text-[0.625rem] uppercase tracking-wide text-zinc-500">{m.label}</div>
                <div className="mt-1 text-xl font-bold text-zinc-100">{m.value}</div>
                <div className="text-[0.6875rem] text-zinc-500">{m.sub}</div>
              </div>
            ))}
          </div>
          ...
          {auditLogs.length === 0 ? <Empty icon="plan" title="No admin actions yet" subtitle="Audit entries appear as admins take actions." /> : auditLogs.map((item) => (
            <div key={item.id} className="rounded-lg border border-zinc-800/60 px-3 py-2.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[0.6875rem] text-zinc-500">{formatDate(item.created_at)}</span>
              </div>
              ...
              <tbody>
                {trialFunnel.map((row) => (
                  <tr key={row.day} className="border-b border-zinc-900/80 text-zinc-300">
                    ...
                    <td className="py-2 pr-4 text-right tabular-nums text-rose-400">{row.activated}</td>
              ...
              <tbody>
                {newVsReturning.map((row) => (
                  <tr key={row.day} className="border-b border-zinc-900/80 text-zinc-300">
                    ...
                    <td className="py-2 text-right tabular-nums text-rose-400">{row.returning_users}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              {geo.map((row) => (
                <div key={row.domain} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-zinc-800 overflow-hidden">
                    <div className="h-full rounded-full bg-rose-500" style={{ width: `${Math.round((row.user_count / maxCount) * 100)}%` }} />
                  </div>
            ...
            <table className="min-w-max text-xs">
              <thead className="text-zinc-500">
                <tr className="border-b border-zinc-800/60">
                  ...
                  {featureDays.map((d) => (
                    <th key={d} className="py-2 px-1.5 font-medium text-center text-[0.625rem]">{d.slice(5)}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={feat} className="border-b border-zinc-900/80">
                      ...
                      {featureDays.map((d) => {
                        ...
                        const colors = ['bg-zinc-900', 'bg-rose-950', 'bg-rose-900/40', 'bg-rose-800/50', 'bg-rose-700/50', 'bg-rose-600/60', 'bg-rose-500/60', 'bg-rose-400/70', 'bg-rose-300/70', 'bg-rose-200/80'];
                        return (
                          <td key={d} className="py-1 px-1.5">
                            <div
                              className={`w-8 h-6 rounded text-center leading-6 text-[0.625rem] font-medium ${colors[intensity] || 'bg-zinc-900'} ${v > 0 ? 'text-zinc-200' : 'text-zinc-700'}`}
...
function UsersTab({ notify }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 rounded-lg border border-rose-800/60 bg-rose-900/20 px-4 py-2.5">
          <span className="text-sm text-rose-300">{selectedIds.size} selected</span>
          ...
          <div className="space-y-6">
                    ...
                    >
                      <div className="aspect-[4/5] bg-zinc-950 flex items-center justify-center overflow-hidden">
                        {item.mediaType === 'image' && item.previewUrl ? (
                          ...
                          <div className="space-y-1 px-3 text-center">
                            ...
                            {item.metadata?.model ? (
                              <div className="text-[0.625rem] text-zinc-600 line-clamp-2">{item.metadata.model}</div>
                            ) : null}
                          </div>
                        )}
                      </div>
                      <div className="p-2.5 space-y-1.5">
                        ...
                        {item.metadata?.outputCount !== undefined || item.metadata?.error ? (
                          <div className="text-[0.625rem] text-zinc-500 line-clamp-2">
                            {item.metadata?.outputCount !== undefined ? `${item.metadata.outputCount} outputs` : ''}
                            {item.metadata?.outputCount !== undefined && item.metadata?.error ? ' · ' : ''}
                            {item.metadata?.error || ''}
                          </div>
                        ) : null}
                        <div className="text-[0.625rem] text-zinc-600">{formatDate(item.createdAt)}</div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  >
                    ...
                    <div className="p-4 space-y-3 overflow-y-auto">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[0.6875rem] text-zinc-500 ml-auto">{formatDate(lightboxItem.createdAt)}</span>
                      </div>
                      {lightboxItem.prompt ? (
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-[0.6875rem] font-semibold uppercase tracking-wider text-zinc-500">Prompt</span>
                            <Btn
                              variant="secondary"
                      ...
                      {lightboxItem.metadata?.model ? (
                        <div className="text-[0.6875rem] text-zinc-500">Model: <span className="text-zinc-400">{lightboxItem.metadata.model}</span></div>
                      ) : null}
                    </div>
                  </div>
                </div>
                      ...
                      {(allLibraryItems || []).map((item) => (
                        <div key={`all-${item.id}`} className="overflow-hidden rounded-xl border border-zinc-800/60 bg-zinc-900/40 cursor-pointer hover:border-zinc-600/60 transition-colors" onClick={() => { setShowAllLibrary(false); setLightboxItem(item); }}>
                          ...
                          <div className="p-2 space-y-1">
                            ...
                            <div className="text-[0.625rem] text-zinc-500 line-clamp-2">{item.prompt || 'No prompt'}</div>
                            <div className="text-[0.625rem] text-zinc-600">{formatDate(item.createdAt)}</div>
                          </div>
                        </div>
                      ))}
            ...
            <Card className="space-y-4">
              ...
              {sentMessages.length > 0 && (
                <div className="space-y-2 pt-1 border-t border-zinc-800/60">
                  <p className="text-[0.6875rem] font-semibold uppercase tracking-wider text-zinc-500">Previously sent</p>
                  {sentMessages.map((m) => (
                    <div key={m.id} className="rounded-lg border border-zinc-800/60 bg-zinc-900/30 px-3 py-2 space-y-0.5">
                      ...
                      <div className="flex items-center gap-2 text-[0.625rem] text-zinc-600">
            ...
            <Card className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <a href="https://t.me/Kyros_Studio" target="_blank" rel="noreferrer" className="text-xs text-rose-400 hover:text-rose-300">Telegram Support</a>
              </div>
              <textarea
                ...
                className="min-h-[80px] w-full rounded-lg border border-zinc-700/80 bg-zinc-900/60 px-3 py-2.5 text-sm text-zinc-100 outline-none focus:border-rose-500/70 focus:ring-1 focus:ring-rose-500/20 placeholder:text-zinc-600"
              />
                  ...
                  {supportNotes.map((note) => (
                    <div key={note.id} className="rounded-lg border border-zinc-800/60 p-3">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-[0.6875rem] text-zinc-600">{formatDate(note.created_at)}</span>
                      </div>
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                {selectedActivity.length ? selectedActivity.map((item) => (
                  <div key={`${item.type}-${item.id}`} className="rounded-lg border border-zinc-800/60 p-3">
                    ...
                    {item.details ? <pre className="mt-1.5 overflow-x-auto whitespace-pre-wrap rounded-md bg-zinc-950/70 p-2 text-[0.6875rem] text-zinc-400">{item.details}</pre> : null}
                  </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Total Paid Out" value={`$${(summary?.paidTotal ?? 0).toFixed(2)}`} sublabel="All-time commissions paid" accent="text-rose-400" />
      </div>
