    ...
    <div className="space-y-5 animate-in">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="h-8 w-48 rounded-lg border border-zinc-700/60 bg-zinc-800/60 px-3 text-xs text-zinc-200 placeholder-zinc-600 inset-depth focus:outline-none focus:ring-2 focus:ring-blue-500/20"
        />
    ...
    <div className={`rounded-2xl overflow-hidden bg-zinc-900 border shadow-lg hover:shadow-xl transition-all duration-300 group ${
      ...
      <div className="px-3 py-2.5 space-y-1">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[10px] text-zinc-500">{formatDate(v.createdAt)}</span>
          <Badge color="zinc">{MODEL_MAP[v.model]?.label || v.model}</Badge>
          {v.duration && <span className="text-[10px] text-zinc-600">{v.duration}s</span>}
        </div>
        {v.error && <p className="text-[10px] text-red-400 truncate">{v.error}</p>}
      </div>
