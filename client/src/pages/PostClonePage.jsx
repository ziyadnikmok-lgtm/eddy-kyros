...
const DNA_LABELS = {
  ...
  camera: { label: 'Camera', color: 'text-rose-400', dot: 'bg-rose-400' },
  pose: { label: 'Pose', color: 'text-rose-400', dot: 'bg-rose-400' },
  ...
  wig: { label: 'Wig', color: 'text-rose-400', dot: 'bg-rose-400' },
};
          ...
          <Card className="space-y-4">
            ...
            {inputMode === 'single' ? (
              <>
                ...
                />
                {cosplayMode && (
                  <p className="text-[0.625rem] text-rose-400/80 -mt-2">Keeps wig color &amp; styling from the source instead of your character's natural hair.</p>
                )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {ASPECT_RATIOS.map((ratio) => (
                        <button
                          ...
                          className={`rounded-lg px-3 py-1.5 text-xs font-medium transition cursor-pointer ${aspectRatio === ratio ? 'bg-rose-500 text-white' : 'bg-zinc-700/60 text-zinc-200 hover:bg-zinc-600/80'}`}
                        >
                          {ratio}
                        </button>
                      ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {RESOLUTION_TIERS.map((tier) => (
                        <button
                          ...
                          className={`rounded-lg px-3 py-1.5 text-xs font-medium transition cursor-pointer ${resolutionTier === tier ? 'bg-rose-500 text-white' : 'bg-zinc-700/60 text-zinc-200 hover:bg-zinc-600/80'}`}
                        >
                          {tier}
                        </button>
                      ))}
          ...
          <Card className="!p-0 overflow-hidden">
            ...
            >
              ...
              <span className="flex items-center gap-2">
                {sessionStatus?.status === 'active' && (
                  <span className="text-green-400 text-[0.625rem]">{sessionStatus.message}</span>
                )}
                {sessionStatus?.status === 'expired' && (
                  <span className="text-red-400 text-[0.625rem]">Expired</span>
                )}
            ...
            {showSession && (
              <div className="px-3 pb-3 space-y-2 border-t border-zinc-700/60 pt-2">
                ...
                {sessionInfo?.hasInstagramSession && (
                  <div className="text-[0.6875rem] text-zinc-500 font-mono">{sessionInfo.maskedValue}</div>
                )}
                <Input
            ...
            <Card className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={selectAll} className="text-[0.6875rem] text-rose-400 hover:text-rose-300 transition-colors cursor-pointer">Select All</button>
                  <span className="text-zinc-700">|</span>
                  <button type="button" onClick={deselectAll} className="text-[0.6875rem] text-zinc-500 hover:text-zinc-400 transition-colors cursor-pointer">Deselect All</button>
                ...
                {fetchedPosts.map((post, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={cn(
                        'absolute top-1.5 right-1.5 w-5 h-5 rounded-md flex items-center justify-center text-[0.625rem] font-bold transition-colors',
                        isSelected
                      ...
                      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent px-1.5 pb-1.5 pt-4">
                        <div className="flex items-center gap-1">
                          <Badge color="zinc" className="!text-[0.5625rem] !px-1 !py-0">{post.type}</Badge>
                          {post.slideCount > 1 && (
                            <span className="text-[0.5625rem] text-zinc-400">{post.slideCount} slides</span>
                          )}
                        </div>
                      </div>
                    </button>
          ...
          {Array.isArray(result) && result.length > 0 && result.map((post, idx) => {
            ...
            return (
              <Card key={`${post.sourceUrl || 'post'}-${idx}`} className="space-y-3">
                ...
                {post.sourceUrl && <p className="text-[0.6875rem] text-zinc-500 truncate">{post.sourceUrl}</p>}
                ...
                {savingFocus === idx && (
                  <div className="flex items-center gap-2 bg-zinc-800/50 rounded-lg p-2">
                    <input
                      ...
                      className="flex-1 h-8 rounded-md border border-zinc-700/60 bg-zinc-900/50 px-2.5 text-xs text-zinc-100 placeholder-zinc-500 outline-none focus:border-rose-500/70"
                    />
                        ...
                        return (
                          <div key={key} className="bg-zinc-800/40 rounded-lg p-2">
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              <span className={cn('text-[0.625rem] font-medium uppercase tracking-wider', meta.color)}>{meta.label}</span>
                            </div>
                            <p className="text-[0.6875rem] text-zinc-300 leading-relaxed line-clamp-3">{value}</p>
                          </div>
                        );
                      })}
                    </div>
              ...
              <Card key={entry.id} className="!p-3 space-y-2">
                <div className="flex gap-1.5 overflow-x-auto pb-1">
                  ...
                  {(entry.galleryIds || []).length > 4 && (
                    <div className="w-14 h-[70px] rounded-md bg-zinc-800/60 border border-zinc-700/40 flex items-center justify-center flex-shrink-0">
                      <span className="text-[0.625rem] text-zinc-500">+{entry.galleryIds.length - 4}</span>
                    </div>
                  )}
                </div>
                {entry.sourceUrl && (
                  <a
                    ...
                    className="block text-[0.6875rem] text-rose-400 hover:text-rose-300 truncate transition-colors"
                  >
                    {entry.sourceUrl}
                  </a>
                )}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    {entry.slideCount > 1 && (
                      <span className="text-[0.625rem] text-zinc-500">{entry.slideCount} slides</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[0.625rem] text-zinc-600">{new Date(entry.createdAt).toLocaleDateString()}</span>
                    {(entry.galleryIds || []).length > 0 && (
                      <button
                        ...
                        className="text-[0.625rem] text-zinc-600 hover:text-rose-400 transition-colors cursor-pointer"
                      >
                        Download ZIP
                      </button>
                    )}
                    <button
                      ...
                      className="text-[0.625rem] text-zinc-600 hover:text-red-400 transition-colors cursor-pointer"
                    >
                      Remove
                    </button>
                  </div>
