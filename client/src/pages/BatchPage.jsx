      ...
      <div>
        ...
        {loadingGallery ? (
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 auto-rows-[120px] gap-2 max-h-[50vh] overflow-y-auto rounded-xl border border-zinc-800 bg-[#111] p-2">
            {/* Upload button */}
            <label className="w-full h-full flex flex-col items-center justify-center rounded-lg border border-dashed border-zinc-700 bg-zinc-900/70 text-zinc-400 cursor-pointer hover:border-rose-500/70 hover:text-zinc-200 transition">
              <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={handleUpload} />
              <span className="text-xl leading-none">+</span>
              <span className="text-[0.625rem] mt-1">Upload</span>
            </label>
            {/* Uploaded image preview */}
            {uploadedImg && (
              <button
                onClick={() => { setSelectedId(null); setUploadedImg(uploadedImg); }}
                className="w-full h-full overflow-hidden rounded-lg border-2 border-rose-500 ring-1 ring-rose-500/40"
              >
            ...
            {images.map(img => (
              <button
                ...
                className={`w-full h-full overflow-hidden rounded-lg border transition duration-150 ${selectedId === img.id && !uploadedImg ? 'border-rose-500 ring-1 ring-rose-500/40' : 'border-zinc-800 hover:border-zinc-600'}`}
              >
        ...
        <div className="flex flex-wrap gap-1.5">
          {REFORMAT_RATIOS.map(({ value, label, desc }) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition cursor-pointer ${targetRatio === value ? 'bg-rose-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'}`}
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {results.map((r, i) => (
          <div
            ...
            className={`cursor-pointer rounded-xl transition-all duration-150 ${selectedIdx === i ? 'ring-2 ring-rose-500 ring-offset-2 ring-offset-zinc-900' : 'hover:opacity-90'}`}
          >
            <ImageCard
              src={r.image?.base64Data ? undefined : (r.galleryUrl || (r.galleryId || r.imageId ? `/api/gallery/${r.galleryId || r.imageId}/image` : undefined))}
...
export default function BatchPage() {
  ...
  const successfulResults = (job?.results || []).filter((r) => r?.success && (r?.image?.base64Data || r?.hasImage || r?.galleryUrl));
  const jobImageUrls = successfulResults.map((r) =>
    r.image?.base64Data
      ? `data:${r.image?.mimeType || 'image/png'};base64,${r.image?.base64Data}`
      : (r.galleryUrl || `/api/gallery/${r.galleryId || r.imageId}/image`)
  );
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none sm:flex-wrap sm:overflow-visible sm:pb-0 sm:gap-2">
            {[['variation', 'Variation'], ['multi', 'Multi'], ['override', 'Override'], ['edit', 'Edit'], ['content-mix', 'Mix'], ['reformat', 'Reformat']].map(([m, label]) => (
              <button key={m} onClick={() => update({ mode: m })}
                className={`rounded-lg px-3 py-2 text-xs sm:text-sm font-medium transition cursor-pointer whitespace-nowrap flex-shrink-0 ${mode === m ? 'bg-rose-600 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'}`}>
                {label}
              </button>
            ))}
          </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ASPECT_RATIOS.map((ar) => (
                  <button
                    ...
                    className={`rounded-md px-2.5 py-1.5 text-xs font-medium transition cursor-pointer ${aspectRatio === ar ? 'bg-rose-600 text-white' : 'bg-zinc-700/60 text-zinc-400 hover:bg-zinc-600 hover:text-zinc-200'}`}
                  >
                    {ar}
                  </button>
                ))}
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <span className="text-[0.625rem] text-zinc-600 mt-0.5 block">Lower = more consistent</span>
              </div>
              <div>
                <Slider label="Temp Max" value={tempMax} onChange={(v) => update({ tempMax: v })} min={0} max={2} step={0.05} />
                <span className="text-[0.625rem] text-zinc-600 mt-0.5 block">Higher = more creative</span>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {charDetail.references.map((r) => (
                        <button key={r.id} onClick={() => toggleOverrideRef(si, r.id)}
                          className={`text-xs rounded-md px-2 py-1 transition cursor-pointer ${set.referenceIds.includes(r.id) ? 'bg-rose-600 text-white' : 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600'}`}>
                          {r.category}
                        </button>
                      ))}
                    </div>
            ...
            <div>
              ...
              {loadingEditGallery ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 auto-rows-[160px] gap-2 max-h-[55vh] overflow-y-auto pr-1 rounded-xl border border-zinc-800 bg-[#111] p-3">
                  <label className="group relative flex w-full h-full cursor-pointer items-center justify-center rounded-xl border border-dashed border-zinc-700/80 bg-zinc-900/70 text-zinc-400 transition hover:border-rose-500/70 hover:bg-zinc-800/80 hover:text-zinc-200">
                    <input
                      type="file"
                  ...
                  {[...editUploadImages, ...editGalleryImages].map((img) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative w-full h-full overflow-hidden rounded-xl border bg-zinc-950 transition duration-200 hover:scale-[1.02] hover:shadow-[0_0_24px_rgba(59,130,246,0.2)] ${isSelected ? 'border-2 border-rose-500 shadow-[0_0_18px_rgba(59,130,246,0.35)]' : 'border-zinc-800'}`}
                      >
                  ...
                  return (
                    <div key={cat.key} className="flex items-center gap-3">
                      ...
                      <input type="range" min={0} max={100} step={5} value={pct}
                        ...
                        className="flex-1 accent-rose-500 h-1.5 rounded-full appearance-none bg-zinc-700 cursor-pointer" />
      ...
      <Card className="space-y-4">
        ...
        {queueStats && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {[
              { label: 'Running', value: queueStats.running, color: 'text-rose-400' },
              ...
              { label: 'Queue', value: queueStats.queueDepth, color: 'text-rose-400' },
              ...
              <div key={s.label} className="text-center rounded-lg bg-zinc-800/60 py-2">
                ...
                <div className="text-[0.625rem] text-zinc-500">{s.label}</div>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-1">
          {['', 'running', 'completed', 'failed', 'cancelled'].map(f => (
            <button key={f} onClick={() => setStatusFilter(f)}
              className={`rounded-md px-2.5 py-1 text-[0.6875rem] font-medium transition cursor-pointer ${
                statusFilter === f ? 'bg-rose-600 text-white' : 'bg-zinc-800/60 text-zinc-400 hover:text-zinc-200'
              }`}>
              {f || 'All'}
            </button>
          ))}
                ...
                <div key={h.jobId} className="rounded-lg border border-zinc-700/50 bg-zinc-800/40">
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-[0.625rem] text-zinc-600">{h.completed}/{h.total}</span>
                      {failedCount > 0 && <span className="text-[0.625rem] text-red-400">{failedCount} failed</span>}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {duration !== null && <span className="text-[0.625rem] text-zinc-600">{duration}s</span>}
                  ...
                  {isExpanded && (
                    <div className="px-3 pb-2.5 border-t border-zinc-700/30 pt-2 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[0.625rem] text-zinc-500">{successRate}%</span>
                      </div>
                      {h.config && (
                      ...
                      <div className="flex gap-2">
                        ...
                        {h.status !== 'running' && (
                          <button onClick={() => handleRemoveJob(h.jobId)}
                            className="text-[0.625rem] text-zinc-600 hover:text-red-400 cursor-pointer ml-auto">
                            Remove
                          </button>
                        )}
                      </div>
