...
const VALID_PAGE_IDS = new Set(['generate', 'nsfwGenerate', 'batch', 'auto', 'video', 'videoGallery', 'seedanceVideo', 'seedanceOmni', 'seedreamEdit', 'carousel', 'scene', 'postClone', 'instagramFrames', 'frameLibrary', 'styleLibrary', 'promptBuilder', 'profileAnalyzer', 'storyteller', 'loraDataset', 'library', 'pasteInbox', 'imageEditor', 'characters', 'keys', 'billing', 'referral', 'logs', 'photoMatch', 'poseFix', 'nanoBypass', 'outfitSwap', 'seedreamGenerate', 'outfitSwapSeedream', 'photoMatchSeedream', 'photoMatchNB2', 'pinterestFeed', 'sceneRecreateSeedream', 'poseRemixSeedream', 'eddyGenerate', 'eddy', 'eddyLibrary', 'eddyOutfit', 'eddyPose', 'eddyEnvironment', 'eddyMaxNano', 'eddyMaxOutfit', 'eddyBase', 'eddyBaseLibrary', 'videoLibrary', 'pinterestLibrary', 'eddyCharacter', 'admin', 'settings', 'instagramReel', 'videoEditor']);

function pageFromPathname(pathname) {
  const segment = (pathname || '/').replace(/^\/+|\/+$/g, '') || 'eddy';
  return VALID_PAGE_IDS.has(segment) ? segment : 'eddy';
}
...
export function AppProvider({ children }) {
  ...
  const navigateTo = useCallback((pageId, params = {}) => {
    // Any new page MUST be added to VALID_PAGE_IDS above. An id missing from it lands on
    // Generate with no error, which looks exactly like a broken page.
    if (!VALID_PAGE_IDS.has(pageId) && typeof console !== 'undefined') {
      console.warn(`[nav] unknown page "${pageId}" — add it to VALID_PAGE_IDS. Falling back to generate.`);
    }
