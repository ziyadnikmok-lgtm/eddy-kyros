    ...
    <div className="rounded-xl border border-zinc-800 bg-[#111] p-3">
      {loading ? (
        ...
        <div className="grid [grid-template-columns:repeat(auto-fill,minmax(120px,1fr))] gap-3 max-h-[300px] overflow-y-auto pr-1">
          {allowUpload && (
            <label className="group relative flex aspect-square cursor-pointer items-center justify-center rounded-xl border border-dashed border-zinc-700/80 bg-zinc-900/70 text-zinc-400 transition hover:border-rose-500/70 hover:bg-zinc-800/80 hover:text-zinc-200">
              <input
                type="file"
          ...
          {mergedItems.map((image) => {
            ...
            return (
              <button
                ...
                className={`group relative aspect-square overflow-hidden rounded-xl border bg-zinc-900 transition duration-200 hover:scale-[1.02] hover:shadow-[0_0_24px_rgba(59,130,246,0.2)] ${isSelected ? 'border-2 border-rose-500 shadow-[0_0_18px_rgba(59,130,246,0.35)]' : 'border-zinc-800'}`}
              >
