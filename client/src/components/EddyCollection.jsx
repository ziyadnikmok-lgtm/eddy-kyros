...
export default function EddyCollection({
  ...
  useEffect(() => { setSelected([]); }, [activeFolder]);

  const visible = useMemo(() => {
    const inFolder = activeFolder ? items.filter((i) => i.folderId === activeFolder) : items;
    return oldestFirst ? [...inFolder].sort((a, b) => a.createdAt - b.createdAt) : inFolder;
  }, [items, activeFolder, oldestFirst]);
  ...
  const attachTo = useCallback(async (id, file) => {
    ...
    try {
      ...
      if (describeKind && !hadPrompt) await describe(id, dataUrl);
    } catch (err) {
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <button
          ...
          className={cn('rounded-full border px-3 py-1.5 text-xs font-medium transition cursor-pointer',
            activeFolder === null ? 'border-rose-500 bg-rose-500/15 text-white' : 'border-zinc-700/60 bg-white/[0.02] text-zinc-400 hover:text-white')}
        >
          All ({items.length})
        </button>
        {folders.map((f) => {
          ...
          return (
            <span key={f.id} className="group relative inline-flex">
              <button
                ...
                className={cn('rounded-full border px-3 py-1.5 text-xs font-medium transition cursor-pointer',
                  activeFolder === f.id ? 'border-rose-500 bg-rose-500/15 text-white' : 'border-zinc-700/60 bg-white/[0.02] text-zinc-400 hover:text-white')}
              >
                {f.name} ({count})
              </button>
              <button
